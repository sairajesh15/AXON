...
type FetchResult =
	| { platform: string; problemsSolved: number; streakDays?: number; raw?: any }
	| { error: string };
...
export async function ingestCodingForStudent({
	email,
	platform,
	username,
	rawData,
	...
	const existing = await prisma.codingActivity.findFirst({
		where: { studentId: student.id, platform },
	});
	if (existing) {
		await prisma.codingActivity.update({
			where: { id: existing.id },
			data: { problemsSolved, streakDays },
		});
	} else {
		await prisma.codingActivity.create({
			data: { studentId: student.id, platform, problemsSolved, streakDays },
		});
	}
	...
	if (
		...
		(fetched as any).raw?.data?.matchedUser?.submissionCalendar
	) {
	...
	for (const log of logsToInsert) {
		const normalizedDate = new Date(
			Date.UTC(log.date.getUTCFullYear(), log.date.getUTCMonth(), log.date.getUTCDate()),
		);
...
export async function addOrUpdatePlatformUsername({
	email,
	platform,
	username,
}: {
	...
	const existing = await prisma.platformUsername.findFirst({
		where: { studentId: student.id, platform },
	});
	if (existing) {
...
export async function listPlatformUsernames() {
	return await prisma.platformUsername.findMany({
...
export async function pollAllPlatformMappings({ rateMs = 300 } = {}) {
	...
	for (const m of mappings) {
		try {
			const r = await ingestCodingForStudent({
